...
func (j *CheckClientIpJob) Run() {
	...
	if iplimitActive && !f2bInstalled {
		logger.Warning("[LimitIP] Fail2Ban is not installed, Please install Fail2Ban from the x-ui bash menu.")
	}

	j.processObserved(observed, iplimitActive && f2bInstalled)
}
...
func (j *CheckClientIpJob) collectFromOnlineAPI() (map[string]map[string]int64, bool) {
	...
	for _, user := range onlineUsers {
		for _, entry := range user.IPs {
			...
			observed[user.Email][entry.IP] = now
		}
	}
	return observed, true
}

func (j *CheckClientIpJob) processObserved(observed map[string]map[string]int64, enforce bool) bool {
...
func (j *CheckClientIpJob) processLogFile() bool {
	...
	for email, ipTimestamps := range inboundClientIps {
		...
		shouldCleanLog = j.updateInboundClientIps(clientIpsRecord, email, ipsWithTime, true) || shouldCleanLog
	}
